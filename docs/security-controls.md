...
| **Application layer attacks (SQLi, XSS)** | Inspect and block malicious HTTP/HTTPs payloads before they reach the compute tier. | AWS WAF | AWS WAF Web ACL containing AWS Managed Core Rule Set (blocking SQLi/XSS). | CloudWatch logs / WAF metrics showing a blocked simulated SQLi request. |
| **Network and Transport layer attacks (DDoS)** | Ensures the underlying availability and reachability of the Application Load Balancer and Route 53 infrastructure. | AWS Shield Standard | Automatically drops volumetric and state-exhaustion attacks such as SYN floods, UDP reflection, and ICMP floods before they can saturate VPC bandwidth. | CloudWatch logs / WAF metrics showing blocked DDoS attacks. <br> AWS WAF logs validate that malicious payloads are intercepted. |
