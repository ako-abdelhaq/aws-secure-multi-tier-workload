...
| **Application layer attacks (SQLi, XSS)** | Inspect and block malicious HTTP/HTTPs payloads before they reach the compute tier. | AWS WAF | AWS WAF Web ACL containing AWS Managed Core Rule Set (blocking SQLi/XSS). | CloudWatch logs / WAF metrics showing a blocked simulated XSS, SQLi, Generative Path Traversal requests. |
| **Network and Transport layer attacks (DDoS)** | Ensures the underlying availability and reachability of the Application Load Balancer and Route 53 infrastructure. | AWS Shield Standard | Automatically drops volumetric and state-exhaustion attacks such as SYN floods, UDP reflection, and ICMP floods before they can saturate VPC bandwidth. | The only "proof" that Shield Standard worked is that the backend application didn't crash. <br> **Note:** If we were using Shield Advanced, we would gain access to customized mitigation metrics, CloudWatch alarms (`DDoSDetected`). |
